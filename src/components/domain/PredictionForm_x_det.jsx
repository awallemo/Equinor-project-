...
import { drawRegressionLine_a_d_x, drawBoundaries_x} from "../../utils/Predict_x_from_y";
...
export default class PredictionFormXDet extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
      y: '',
    ...
    handle_y_Change(event) {
      this.setState({
        ...
        p50: parseFloat(drawRegressionLine_a_d_x(this.props.data[0]["name"], event.target.value),0).toFixed(2),
        p90_min: parseFloat(drawBoundaries_x(this.props.data[0]["name"], event.target.value,0)[0]).toFixed(2),
        p90_max: parseFloat(drawBoundaries_x(this.props.data[0]["name"], event.target.value,0)[1]).toFixed(2)
