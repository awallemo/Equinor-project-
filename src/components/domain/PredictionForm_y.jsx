...
import { drawRegressionLine_all, drawBoundaries} from "../../utils/CsvParser";
...
export default class PredictionFormY extends React.Component {
    constructor(props) {
      super(props);
      this.state = {
      x: '',
    ...
    handle_x_Change(event) {
      ...
      this.setState({
        ...
        p50: parseFloat(drawRegressionLine_all(this.props.data[0]["name"], event.target.value)).toFixed(2),
        p90_min: parseFloat(drawBoundaries(this.props.data[0]["name"], event.target.value)[0]).toFixed(2),
        p90_max: parseFloat(drawBoundaries(this.props.data[0]["name"], event.target.value)[1]).toFixed(2)
      });
    }
